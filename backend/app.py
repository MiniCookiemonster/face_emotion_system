...
from shape_key_driver import apply_shape_keys, load_shape_key_data
...
# ============================================================
# ICT-FaceKit 六种基础情绪到 shape key 的映射
# 注意：
# 1. 这里使用的是 ICT / ARKit 风格的细分表情 key。
# 2. identity000、identity001 ... 这类 key 是身份/脸型，不参与表情驱动。
# 3. 如果你的 shape_key_deltas.json 里没有某个 key，程序会自动跳过并打印提示。
# ============================================================

ICT_EMOTION_PRESETS: dict[str, dict[str, float]] = {
    "Happy": {
        "mouthSmile_L": 0.85,
        "mouthSmile_R": 0.85,
        "cheekSquint_L": 0.45,
...
def build_shape_key_weights_from_frontend(
    effective_emotions: dict[str, float],
    available_keys,
) -> dict[str, float]:
    """
    把前端传来的六种情绪强度转换成 ICT-FaceKit 的 shape key 权重。
    ...
    for emotion_name, emotion_intensity in effective_emotions.items():
        ...
        preset = ICT_EMOTION_PRESETS.get(emotion_name)
        ...
        for key_name, preset_value in preset.items():
            ...
            if real_key_name is None:
                print(f"当前 shape_key_deltas.json 中找不到 ICT shape key: {key_name}")
                continue
...
def build_result_vertices(
    ...
    if shape_key_data is not None:
        ...
        print("没有加载到 shape_key_deltas.json，当前只会使用 deform_mesh 的基础变形。")

    # ============================================================
    # 重要：
    # 以前只有 Happy / Sad 由 shape key 驱动，所以其他情绪还交给 deform_mesh。
    # 现在 ICT-FaceKit 六种情绪都走 shape key，因此这里把残余情绪全部清零，
    # 避免 shape key 变形后又被 deform_mesh 二次拉扯，造成头部/脸型异常。
    # ============================================================
    residual_emotions = {
        "Happy": 0.0,
        "Sad": 0.0,
...
async def deform_api(
    ...
    try:
        ...
        shape_key_entries = get_shape_key_entries(shape_key_data)
        available_shape_keys = list(shape_key_entries.keys()) if shape_key_entries else []

        print("effective_emotions =", effective_emotions)
        print("available_shape_keys =", available_shape_keys[:30], "..." if len(available_shape_keys) > 30 else "")
